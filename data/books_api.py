...
def formatter(inp_data):
    ...
    for i in inp_data:
        res[int(i.ID)] = [i.BOOK_NAME, i.BOOK_ALTER_NAME, i.AUTHOR, i.PUBLISHER, i.DESCRIPTION, i.OUT_DATE,
                          i.OUT_COUNTRY, [int(j) for j in i.GENRES.split("_")], i.FILE_NAME,
                          [int(j) for j in i.CODECS.split("_")], i.CODECS_HASH, i.DOWNLOADS]
    return res
...
def BOOK_api_filter():
    ...
    data = formatter(query_all_books.all())
    if "genres" in right_args and right_args["genres"] != "":
        # Жанры, если что идут через точку с запятой
        buff = right_args["genres"].split("_")
        genres = []
